---
title: Task Properties Settings
source: https://tasknotes.dev/settings/task-properties/
author:
published:
created: 2026-08-05
description: This tab configures all task properties. Each property is displayed as a card containing its configuration options.
tags:
  - clippings
...
> 
> See the [Property Types Reference](https://tasknotes.dev/settings/property-types-reference/) for detailed documentation on the expected data types (text, list, date, etc.) for each frontmatter property.
...
- **Autosuggest Filters**: Filter which files appear when using `[[` wikilink autocomplete Custom fields are most maintainable when they map to repeated workflow decisions (for example `effort`, `owner`, or `client`).
...
See [User Fields Feature Documentation](https://tasknotes.dev/features/user-fields/#file-suggestion-filtering-advanced) for detailed examples.