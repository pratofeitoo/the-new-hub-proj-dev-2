---
title: User Fields
source: https://tasknotes.dev/features/user-fields/
author:
published:
created: 2026-08-05
description: TaskNotes allows you to define your own custom fields for tasks. This feature allows you to add custom data to your tasks and use it for filtering, sorting, and grouping. User f...
tags:
  - clippings
...
User fields are created in the TaskNotes settings, under the "Task Properties" tab. To create a new user field, click the "Add new user field" button. Fields that are not used in filters or grouping still add frontmatter overhead, so this section focuses on fields used in views.
...
- A "Related Document" field filtered by a specific frontmatter property Filtering narrows wikilink suggestions in large vaults.
...
- **Filters only affect autocomplete**: They don't affect the actual field value or validation Autocomplete filtering and stored field values are handled independently.
...
- **Grouping**: Use the Bases group menu to create swimlanes or list groupings based on the user field. To verify a new field is active in workflow, add it to at least one saved view.
...
For example, if you create a user field with the property name "my\_field", the data for that field will be stored in the frontmatter as follows:
