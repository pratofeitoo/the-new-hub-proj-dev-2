---
title: Inline Task Integration
source: https://tasknotes.dev/features/inline-tasks/
author:
published:
created: 2026-08-05
description: TaskNotes integrates with the Obsidian editor to allow task management directly within notes. This is achieved through interactive widgets, a conversion feature for checkboxes,...
tags:
  - clippings
updated: 2026-08-05T03:42
---
...
![Task Link Overlays in Live Preview mode](https://tasknotes.dev/assets/2025-07-17_21-03-55.png)

*Task link overlays in Live Preview mode show interactive widgets with status, dates, and quick actions*

![Task Link Overlays in Source mode](https://tasknotes.dev/assets/2025-07-17_21-04-24.png)
...
- **Action Menu**: Ellipsis icon (shown on hover) provides additional task actions. The widget exposes common task actions at the link location, so status and metadata can be updated in-place.
...
- **Natural language processing** can extract dates and metadata from plain text (if enabled) Conversion preserves surrounding formatting, including callouts, outlines, and nested lists.
...
- Very long task titles (over 200 characters) For large notes, converting a small section first helps validate folder and filename settings.
...
- **User-Defined Fields**: Custom fields can be assigned using configured triggers (e.g., `effort: high`). Supports quoted values for multi-word entries. NLP parses common patterns during capture; fields can be refined in the modal afterward.
