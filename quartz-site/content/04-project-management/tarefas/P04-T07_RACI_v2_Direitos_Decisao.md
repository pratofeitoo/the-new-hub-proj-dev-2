---
title: P04-T07 — RACI v2 + matriz direitos de decisão + incidentes
task_id: P04-T07
phase: P04
status: pendente
priority: critica
area: governance-legal
layer: refining
owner:
  - Jurídico (a designar)
  - PF Rezende
gap_ids:
  - GOV-008
  - STR-007
dependencies:
  - P02-T06
  - P04-T01
target_file: "01-blueprint/governanca-juridico/HUB_Blueprint_Governanca_e_Juridico.md"
related_notes:
  - "[[04-project-management/planos-fase/P04_Governanca_Confianca]]"
  - "[[00-project-control/registro-lacunas/lacunas/GOV-008]]"
  - "[[00-project-control/registro-lacunas/lacunas/STR-007]]"
created: 2026-08-26
tags:
  - task
  - fase-P04
---

# P04-T07 — RACI v2 + matriz direitos de decisão + incidentes

## Objetivo
Consolidar RACI v2 sem ambiguidade (1 A por atividade crítica) + direitos de decisão + tratamento de incidentes + sucessão fundador.

## Entregável
RACI v2 em `HUB_Blueprint_Governanca_e_Juridico.md` + `00-project-control/registro-lacunas/lacunas/GOV-008.md`.

## Dependências
- [[04-project-management/tarefas/P02-T06_RACI_Accountable_Unico|P02-T06]]
- [[04-project-management/tarefas/P04-T01_Arquitetura_Entidades|P04-T01]]

## Critério (G04.7)
Nenhum crítico com ambiguidade; decisões críticas com dono não-fundador.

## Registros
- [[00-project-control/registro-lacunas/lacunas/GOV-008]] · [[00-project-control/registro-lacunas/lacunas/STR-007]]
