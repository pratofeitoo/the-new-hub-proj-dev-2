...
| `evidence` | Evidência que sustenta a correção. |
...
Categorias: `terminology`, `key`, `schema`, `formula`, `unit`, `ownership`, `governance`, `integration`, `roadmap`, `duplication`, `evidence`.
...
- Uma cópia em `03-corrected-csv/` só pode divergir após o registro correspondente existir aqui. Nesta fase, todas as 15 cópias são byte-a-byte idênticas às fontes.
