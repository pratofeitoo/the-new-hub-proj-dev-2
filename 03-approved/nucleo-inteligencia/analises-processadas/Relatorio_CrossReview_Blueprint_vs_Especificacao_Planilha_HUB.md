...
| `FLD-042→047` P1-MVP1 ausentes (envelope + IDs reservados) | 6 FLDs criados: `FLD-042 event_id, 043 event_type, 044 schema_version, 045 occurred_at` (N10 envelope) + `FLD-046 recommendation_id` (N12) + `FLD-047 match_id` (N13) | mesmos + `envelope-evento-schema-P03-T03-v1.md` em `01-work/dados-tech-financas/refinamento-modelo-dados/` | `grep FLD-042 = 1` · `FLD-047 = 1` |
| `workday_id` como PK (conflito com `canonical_id`) | Corrigido para `alias (source_system=Workday, source_id) → canonical_id` — nunca FK direta; §1.1 em P03-T02 | `Planilha 05_DICIONARIO FLD-002` + `01-work/dados-tech-financas/refinamento-modelo-dados/especificacao-identidade-P03-T02-v1.md` §1.1 | `grep "canonical_id" >=3` |
