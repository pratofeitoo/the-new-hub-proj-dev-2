...
**Próxima evidência:** para cada segmento priorizado, preencher no log de [[01-work/pesquisa/log-evidencias-GTM|evidências GTM]] a rota, parceiro, status `hipótese` ou `tração`, fonte, link, fallback, responsável e data de revisão.
...
Os exemplos reproduzíveis e os campos mínimos para cenários estão em [[01-work/modelos-financeiros/HUB_Taxonomia_Receita_Reconhecimento_v1|HUB_Taxonomia_Receita_Reconhecimento_v1]]. A tabela deve ser reconciliada com cada linha da matriz de ofertas §2.1 e com os cenários de contrato antes do gate P01.
...
Este documento conecta a arquitetura de ofertas à identidade do projeto, ao método C.A.O.S., aos seis módulos conceituais da plataforma, às três frentes de negócio, à cadeia de dados/valor, à fundação de governança, ao sequenciamento M0–M4 e às hipóteses comerciais em [[01-work/estrategia/HUB_Fundacao_Blueprint_Projeto]]. Destina-se a prevenir colisões de categoria e tornar explícitas as questões de titularidade, preservando espaço para refinamento. Não afirma demanda validada, parceiros confirmados, capacidade de produção, precificação aprovada, ROI certificado por evidências, aprovação financeira ou prontidão de lançamento. Toda decisão não resolvida permanece vinculada a um ou mais gaps registrados e deve ser revisitada antes da promoção de Blueprint para Refinamento ou Aprovação.
