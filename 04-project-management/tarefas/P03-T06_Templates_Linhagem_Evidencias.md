...
- **Entregável produzido:** [[01-work/refinamento-modelo-dados/templates-linhagem-evidencias-P03-T06-v1|templates-linhagem-evidencias-P03-T06-v1.md]] — templates `origem→métrica→ação→resultado→valor` com campos `run_id`/`formula_version`/`evidence_id` + registro evidências (`origin/captured_at/consent/reviewer/quality`) + 1 caminho financeiro ponta a ponta `PES-02 (3.2→4.1) → PRO-05 → RH-06 (45d→32d) → FIN-01 R$ 18.500 influenciado` com pseudocódigo reprodutível e regra anti-dupla contagem.
