...
layer: blueprint
area: business-model
source_task: "[[04-project-management/tasks/BP-001_HUB_Offer_and_Revenue_Architecture]]"
created: 2026-08-21
updated: 2026-08-21
...
Os limites das ofertas devem ser registrados em uma matriz oferta-comprador-capacidade: quem assina, quem paga, quem participa, o que é entregue, quais módulos de plataforma são usados, qual parceiro é necessário, que evidência pode ser produzida e qual unidade assume o risco. Essa matriz é o refinamento exigido para **STR-002** e deve ser reconciliada com o trabalho de segmentação de portfólio em [[04-project-management/tasks/BP-007_HUB_Go_To_Market_and_Partnerships]] (dependência do BP-007; não infer aqui seus segmentos finais).
...
Este documento conecta a arquitetura de ofertas à identidade do projeto, ao método C.A.O.S., aos seis módulos conceituais da plataforma, às três frentes de negócio, à cadeia de dados/valor, à fundação de governança, ao sequenciamento M0–M4 e às hipóteses comerciais em [[01-blueprint/strategy/HUB_Project_Blueprint_Foundation]]. Destina-se a prevenir colisões de categoria e tornar explícitas as questões de titularidade, preservando espaço para refinamento. Não afirma demanda validada, parceiros confirmados, capacidade de produção, precificação aprovada, ROI certificado por evidências, aprovação financeira ou prontidão de lançamento. Toda decisão não resolvida permanece vinculada a um ou mais gaps registrados e deve ser revisitada antes da promoção de Blueprint para Refinamento ou Aprovação.
